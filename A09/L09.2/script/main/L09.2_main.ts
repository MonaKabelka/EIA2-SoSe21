...
namespace BeesMeadow {
    ...
    function createFlowers(): void {
        let xPos: number = 0;
        do {
            flowers.push(new Flower(Math.floor(Math.random() * 3) + 1, xPos, 120 + (crc2.canvas.height * goldenCut), crc2.canvas.height));
    ...
    function createBees (_nBee: number): void {
        for (let index: number = 0; index < _nBee; index++) {
            ...
            bee.push(new Bee({x: crc2.canvas.width / 2, y: crc2.canvas.height * 0.8}, { x: randomVelocityX, y: randomVelocityY }, {x: randomScaleX, y: randomScaleY}));
        } 
    }
    ...
    function animate(): void {
        ...
        for (let index: number = 0; index < bee.length; index ++) {
            bee[index].update();  
        }
